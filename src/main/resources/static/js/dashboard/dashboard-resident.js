...
//     {id:102, title:"단지 내 좋은 배달 맛집 추천해요", categoryName:"정보", boardCode:"square"},
//     {id:101, title:"주말 바베큐 같이 하실 분 구합니다", categoryName:"자유", boardCode:"square"}
// ];
...
// ];
// var MOCK_BILLING = {billingMonth:"2월", status:"UNPAID", totalAmount:245600, dueDate:"2026-02-28"};
// var MOCK_VEHICLES = [
//     {carNumber:"12가 3456", status:"승인완료"},
//     {carNumber:"34나 7890", status:"승인대기"}
// ];
// var MOCK_TICKETS = {dayTicket:3, hourTicket:7};
// var MOCK_RESERVATIONS = [
//     {id:201, title:"테니스장 A코트", status:"확정"},
//     {id:195, title:"게스트하우스 101호", status:"승인대기"}
// ];
// var MOCK_INQUIRIES = [
//     {id:25, title:"이사 당일 사다리차 이용 시간 문의", status:"미답변"},
//     {id:22, title:"입주민 카드 발급 방법 알려주세요.", status:"답변완료"},
//     {id:19, title:"커뮤니티 센터 헬스장 등록 비용", status:"답변완료"}
// ];
// var MOCK_COMPLAINTS = [
//     {id:10, title:"층간소음 관련하여 중재 요청드립니다.", isSecret:true, status:"확인중"},
//     {id:8, title:"복도 전등이 깜빡거려요.", isSecret:false, status:"답변완료"},
//     {id:5, title:"지하주차장 물 고임 현상", isSecret:false, status:"접수완료"}
// ];
// var MOCK_SCHEDULES = [
...
//     var st = data.status==='UNPAID' ? '미납' : '납부완료';
//     var dateStr = data.dueDate ? data.dueDate.replace(/-/g, '년 ').replace(/년 (\d+)년 (\d+)/, '년 $1월 $2일') : '-';
//     el.innerHTML =
//         '<div style="display:flex; justify-content:flex-end;">'+
//         '<span class="billing-status-badge '+data.status+'">'+st+'</span>'+
//         '</div>'+
//         '<div class="billing-card-row-wrap">'+
//         '<div class="billing-card-row">'+
//         '<span class="billing-card-row-label">'+data.billingMonth+' 관리비</span>'+
//         '<span class="billing-card-row-value">'+comma(data.totalAmount)+'원</span>'+
//         '</div>'+
//         '<div class="billing-card-row">'+
//         '<span class="billing-card-row-label">납부기한</span>'+
//         '<span class="billing-card-row-value">'+dateStr+'</span>'+
//         '</div>'+
//         '</div>';
// }
//
...
//     if (!data||!data.length){el.innerHTML='<div class="loading">등록된 차량 없음</div>';return;}
//     el.innerHTML = data.slice(0,3).map(function(v){
//         var cls = v.status==='승인완료'?'approved':v.status==='승인대기'?'pending':'rejected';
//         return '<div class="parking-vehicle-item"><span>'+(v.carNumber||v.vehicleNumber||'-')+'</span>'+
//             '<span class="parking-status '+cls+'">'+v.status+'</span></div>';
//     }).join('');
// }
//
// function renderTicket(data) {
//     if (!data) return;
//     document.getElementById('dayTicket').textContent  = data.dayTicket  != null ? data.dayTicket  : '-';
//     document.getElementById('hourTicket').textContent = data.hourTicket != null ? data.hourTicket : '-';
// }
//
...
//         return '<div class="small-item"><span class="small-item-title">'+r.title+'</span>'+
...
//         return '<div class="small-item"><span class="small-item-title">'+i.title+'</span>'+
...
//         return '<div class="small-item"><span class="small-item-title">'+title+'</span>'+
...
// renderBilling(MOCK_BILLING);
// renderVehicle(MOCK_VEHICLES);
// renderTicket(MOCK_TICKETS);
